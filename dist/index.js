        ...
        if (!('subscribe' in this.WebSocket)) {
            ...
            this.WebSocket.subscribe = function (topic, handler) {
                if (typeof handler !== "function") {
                    return Promise$1.reject(new Error("Handler must be a function"));
                }
            ...
            this.WebSocket.unsubscribe = function (topic, handler) {
                if (!handler || typeof handler !== "function") {
                    if (self.subscriptions.has(topic)) {
                        ...
                        return Promise$1.reject(new Error("Supplied topic is not registered for pub/sub: " + topic));
                    }
                }
                if (self.subscriptions.has(topic)) {
                    var st = self.subscriptions.get(topic);
                    ...
                    if (ind >= 0) {
                        ...
                        return Promise$1.reject(new Error("Supplied Handler is not registered for pub/sub for topic: " + topic));
                    }
                }
                else {
                    return Promise$1.reject(new Error("Supplied topic is not registered for pub/sub: " + topic));
